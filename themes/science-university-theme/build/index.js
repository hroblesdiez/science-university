...
class Search {
  ...
  constructor() {
    this.addSearchHTML();
    this.resultsDiv = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#search-overlay__result');
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(suData.root_url + '/wp-json/su/v1/search?term=' + this.searchField.val(), results => {
      this.resultsDiv.html(`
                <div class="row">
                    <div class="one-third">
                    ...
                    <div class="one-third">
                        ...
                        ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs matches that search. <a href="${suData.root_url}/programs">View all programs</a></p>`}
                        ${results.programs.map(item => `<li><a href="${item.url}">${item.title}</a></li>`).join('')}
                        ${results.programs.length ? '</ul>' : ''}
                        <h2 class="search-overlay__section-title">Professors</h2>
                        ${results.professors.length ? '<ul class="professors-card">' : `<p>No professors matches that search. <a href="${suData.root_url}/professors">View all professors</a></p>`}
                        ${results.professors.map(item => `
                        <li class="professor-card__list-item">
                        <a class="professor-card" href="${item.url}">
                            <img class="profesor-card__image" src="${item.image}" alt="">
                            <span class="profesor-card__name">${item.title}</span>
                        </a>
                    </li>
                        `).join('')}
                    ...
                    <div class="one-third">
                        ...
                        ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>No campuses matches that search. <a href="${suData.root_url}/campuses">View all campuses</a></p>`}
                        ${results.campuses.map(item => `<li><a href="${item.url}">${item.title}</a></li>`).join('')}
                        ${results.campuses.length ? '</ul>' : ''}
                        <h2 class="search-overlay__section-title">Events</h2>
                        ${results.events.length ? '' : `<p>No events matches that search. <a href="${suData.root_url}/events">View all events</a></p>`}
...
<div class="search-overlay__top">
    <div class="container">
        ...
        <input type="text" class="search-term" placeholder="What are you looking for?" name="search-term" id="search-term">
...
class Like {
  ...
  createLike(currentLikeBox) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      url: suData.root_url + '/wp-json/su/v1/manageLike',
      type: 'POST',
      data: {
...
class MyNotes {
  ...
  createNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      error: response => {
        if (response.responseText == 'You have reached yur note limit') {
